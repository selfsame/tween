...
^doc "Replace all occurrences of `something` in a list with `replacement`"
...
^doc "Returns true if item is within items"
...
^doc "Returns true if the array xs contains the item x"
...
^doc "Returns true if (pred item) returns true for any item within xs"
(defn any? (pred xs)
...
^doc "Returns true if dictionary `dict` contains key `key`"
...
^doc "Returns true if `(pred item)` returns true for all items within xs"
(defn all? [pred xs]
  ;; short circuit if any (pred item) is false
  (let [cont true
        res  true]
    (do
      (while cont
